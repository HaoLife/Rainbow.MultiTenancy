...
namespace Rainbow.MultiTenancy.AspNetCore.Text
{
    public class FormattedStringValueExtracter
    {
        ...
        public static ExtractionResult Extract(string str, string format, bool ignoreCase = false)
        {
            ...
            for (var i = 0; i < formatTokens.Count; i++)
            {
                ...
                if (currentToken.Type == FormatStringTokenType.ConstantText)
                {
                    ...
                    else
                    {
                        ...
                        result.Matches.Add(new KeyValuePair<string, string>(previousToken.Text, str.Substring(0, matchIndex)));
            ...
            {
                result.Matches.Add(new KeyValuePair<string, string>(lastToken.Text, str));
            }
        ...
        public class ExtractionResult
        {
            ...
            /// </summary>
            public List<KeyValuePair<string, string>> Matches { get; private set; }

            internal ExtractionResult(bool isMatch)
            {
                IsMatch = isMatch;
                Matches = new List<KeyValuePair<string, string>>();
            }
        }
    }
}
